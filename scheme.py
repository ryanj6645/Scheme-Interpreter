...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(evaluated_operator, MacroProcedure):
            return scheme_eval(evaluated_operator.apply_macro(rest, env), env)
        arguments = rest.map(lambda elem: scheme_eval(elem, env))
        return scheme_apply(evaluated_operator, arguments, env)
...
# The not operator cannot be used here because self.bindings.get(symbol, None) has to return None not False
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('The length of formals and vals must be the same')
        child_frame = Frame(self)
        old_formals = formals
        if old_formals != nil:
            counter = 0
            while len(old_formals) > counter:
                counter += 1
...
def do_and_form(expressions, env):
    ...
    if not len(expressions):
        ...
        evaluated_val = scheme_eval(expressions.first, env)
        if scheme_falsep(evaluated_val):
            return False
        else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        bind = Pair(bindings.first.first, bind)
        check_formals(bind)
        values = Pair(scheme_eval(bindings.first.rest.first, env), values)
        bindings = bindings.rest
    return env.make_child_frame(bind, values)
...
def do_define_macro(expressions, env):
    ...
    env.define(expressions.first.first, MacroProcedure(expressions.first.rest, expressions.rest, env))
